...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Construcción del HTML omitted here for brevity if I keep it from before... but I'll write the whole file to be safe.
    const statusColors = {
...
<div style="max-width:480px;margin:0 auto;background:#151A24;border-radius:16px;border:1px solid #1E2533;overflow:hidden;padding:24px;">
  <h1 style="color:white;text-align:center;">ITAMdesk</h1>
  <p>Hola ${userName || "Usuario"},</p>
  <p>Tu ticket <strong>${ticketTitle}</strong> ha sido actualizado.</p>
  ${contentHtml}
  <a href="${siteUrl}" style="display:block;text-align:center;background:#3B82F6;color:white;padding:12px;border-radius:8px;text-decoration:none;">Ver Ticket</a>
</div></body></html>`;
